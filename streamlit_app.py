...
# Set page config
st.set_page_config(
    page_title="Dirk's o3-mini Research Lab",
    page_icon="🧪",
    layout="wide"
)
...
def conduct_research(api_key, topic, focus_areas):
    ...
    lab = AgentLaboratory(api_key=api_key, model_name="o3-mini")
...
# Main app layout
st.title("🧪 Dirk's o3-mini Research Lab")
...
if hasattr(st.session_state, 'selected_research'):
    ...
    st.download_button(
        ...
        data=content,
        file_name=f"research_{result['id']}_{result['topic'][:30]}.md",
        mime="text/markdown"
    )
else:
